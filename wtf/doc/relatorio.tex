...
Para a geração dos dados foi utilizado um gerador aleatorio para dados normais multivariados e treis amostras de parâmetros  foram geradas:
...
Na Figura \ref{fig:densidade_acc} os resultados são bastante semelhantes e, na maioria dos casos com uma tendência ao normalidade, principalmente no caso do modelo Bayesiano e MLP. No entanto, a saída do COMB e SVM não parecem ter uma distribuição normal.
...
A Tabela \ref{tab:test_nemeyi} mostra os resultados obtidos para o \textbf{pvalue} dois e dois . Como pode ser visto o modelo Bayesiano apresenta diferenças significativas em relação aos outros métodos. Os métodos SVM, MLP e  COMB não tem nenhuma diferença significativa.
