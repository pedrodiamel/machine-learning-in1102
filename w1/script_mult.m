...
for kf = 1:k 
    ...
    Xtrai = X(trIdx,:); Wtrai = W(trIdx);
    Xtest = X(teIdx,:); Wtest = W(teIdx);
    ...
    % X~N(mu, st)
    [mu, Sigma] = estimateNGaussian(Xtrai, Wtrai);
    
    modMultBayes.prior = P;
    modMultBayes.mu = mu;
    modMultBayes.Sigma = Sigma;
    ...
    for i=1:Nt
    Ytest(i) = predictN(Xtest(i,:), modMultBayes);        
    end
    toc;
